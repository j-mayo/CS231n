...
def affine_backward(dout, cache):
    ...
    db = dout.T.dot(np.ones((x.shape[0],))) # db = dout * dqdb?
...
def svm_loss(x, y):
    ...
    L = np.maximum(0, x - correct_score.reshape(-1, 1) + 1)
...
def softmax_loss(x, y):
    ...
    x = x - np.max(x, axis=1).reshape(-1, 1) # for numerical instability...
    
    #x -= np.max(x,axis=1).reshape(-1,1)
    #print(x.shape, x1.shape)
    # 왜인지 이걸 사용하면 dx가 오류남, 왜일까
    #prob = np.exp(x - np.max(x, axis=1).reshape(-1, 1))
    # 근데 이걸 쓰면 돌아감 뭐지...?
    prob = np.exp(x)
    # 물론 이걸 써도 돌아가긴 함
    correct = prob[range(num_train), y]

    loss = np.sum(-np.log(correct / np.sum(prob, axis=1))) / num_train

    dx = prob / np.sum(prob, axis=1).reshape(-1, 1)
